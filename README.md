...
- **Public-page transports preserve token quota.** Public PR diffs, commit/compare diff and patch media, explicit-ref content files, workflow-filtered run lists, and shaped run job/step metadata use [token-free GitHub endpoints](https://docs.octopool.dev/token-free.html); supported top-level `gh run list/view` reads prefer public pages before anonymous API and pooled PAT/App fallbacks. `gh release view` uses the anonymous API to preserve exact raw Markdown, consuming anonymous API quota on cache misses; releases never use pooled credentials.
