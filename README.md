...
- **Server and local regex protection.** Import a JSON rules file to replace private terms with approved text or purge matches before supported submissions. The shim rejects unresolved endpoint placeholders and recognizable active rule JSON in supported content; ordinary prose still rewrites normally. The Worker checks read inputs and canonical outbound requests, including derived probes and redirects. Policy failures and uninspectable commands fail closed. This covers supported shim/relay traffic, not arbitrary obfuscation or other network clients. See [CLI protection](docs/cli.md) and [administration](docs/admin.md) for precise limits and trust boundaries.
