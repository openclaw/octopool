...
- **Public-page transports preserve token quota.** Public PR diffs, commit/compare diff and patch media, explicit-ref content files, human workflow-run summaries, and watch job/step metadata use [token-free GitHub endpoints](https://docs.octopool.dev/token-free.html). Machine `gh run list/view --json` exports instead use shared exact REST reads for native field semantics, with lazy workflow-name metadata. `gh release view` uses the anonymous API to preserve exact raw Markdown, consuming anonymous API quota on cache misses; releases never use pooled credentials.
