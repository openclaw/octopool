...
- **Cached bodies reduce repeated downloads.** Fresh edge or D1 hits reuse stored content; visibility, membership, and revalidation checks can still contact GitHub. When upstream token-free and pooled backends are unavailable or rate-limited, Octopool can also serve a bounded stale public cache entry instead of forcing the caller back to GitHub.
- **Public-page transports preserve token quota.** Public PR diffs, commit/compare diff and patch media, explicit-ref content files, human workflow-run summaries, and watch job/step metadata use [token-free GitHub endpoints](https://docs.octopool.dev/token-free.html). Machine `gh run list/view --json` exports instead use shared exact REST reads for native field semantics, with lazy workflow-name metadata. Release views requesting only tag, URL, publication time, and draft/prerelease flags can use public pages; names, raw Markdown, creation times, and release lists retain the exact anonymous API. Releases never use pooled credentials.
...
- **Local writes, controlled fallback.** The CLI is a drop-in `gh` shim. Safe read-shaped calls try Octopool first, so the server owns cache, app/PAT routing, and pool policy. Guarded native fallback pins and checks supported maintainer reads and PR lifecycle actions; writes retain your local GitHub identity without disabling outbound protection.
- **Server and local regex protection.** Import a JSON rules file to replace private terms with approved text or purge matches before submissions. Modeled content and lifecycle commands retain strict structural snapshots; evolving native `gh` commands receive best-effort filtering across visible arguments, declared piped JSON/text, and `--input` snapshots instead of failing merely because Octopool does not know the command shape yet. Policy failures still block, and the Worker checks read inputs plus canonical outbound requests. Interactive/deferred content, arbitrary obfuscation, and other network clients remain outside the guarantee. See [CLI protection](docs/cli.md) and [administration](docs/admin.md) for precise limits and trust boundaries.
...
octopool stats     # cached-body reuse, top routes, per-caller usage
