...
func verifiedPRCheckMetadata(ctx context.Context, client ghRelayClient, repo, sha string, runHeaders, catalogueHeaders map[string]string, contexts []prCheckContext) (map[int64]prCheckMetadata, error) {
	...
	err := relayReadPair(ctx, func(ctx context.Context) error {
		runs, err := relayCompleteCollection(ctx, client, ghAPIRequest{
			method: "GET", path: repoPath(repo, "actions", "runs"), headers: runHeaders,
		...
		var err error
		names, err = relayPRCheckWorkflowNames(ctx, client, repo, catalogueHeaders)
		return err
	})
	if err != nil {
		return nil, err
	...
	for suiteID := range needed {
		...
		if !found || association.event == "" {
			return nil, localFallbackError{Reason: "missing workflow association for GitHub Actions check suite"}
		}
		missingName = missingName || names[association.workflowID] == ""
	}
	// A new workflow can appear before the shared catalogue expires. Retry the
	// catalogue once, only if every run association exists and it was not live.
	if missingName && relayReadHeaders("GET", catalogueHeaders)["cache-control"] != "max-age=0" {
		names, err = relayPRCheckWorkflowNames(ctx, client, repo, map[string]string{"cache-control": "max-age=0"})
		if err != nil {
			return nil, err
		}
	}
	for suiteID := range needed {
		association := metadata[suiteID]
		association.workflowName = names[association.workflowID]
		if association.workflowName == "" {
			return nil, localFallbackError{Reason: "missing workflow association for GitHub Actions check suite"}
		}
...
func relayPRCheckWorkflowNames(ctx context.Context, client ghRelayClient, repo string, headers map[string]string) (map[int64]string, error) {
