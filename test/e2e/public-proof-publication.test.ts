...
describe("native public proof publication", () => {
  it.each(["read failure", "malformed JSON"])(
    "still observes and publishes after optional edge %s",
    async (kind) => {
      const actual = caches.default;
      vi.stubGlobal("caches", {
        default: {
          match: async () => {
            if (kind === "read failure") throw new Error("synthetic edge read failure");
            return new Response("not JSON");
          },
          put: actual.put.bind(actual),
          delete: actual.delete.bind(actual),
        },
      });
      const response = positive();
      const result = await observeAnonymousPublicRepo(env, route, async () => {
  ...
  it("skips optional acquisition and publication for a recent positive edge receipt", async () => {
    await observeAnonymousPublicRepo(env, route, async () => positive());
    const stored = await proof();
    const inaccessible = new Proxy(env, {
      get(target, key) {
        if (key === "DB" || key === "POOL_COORDINATOR")
          throw new Error("Recent proof added authority call");
        return Reflect.get(target, key);
      },
    });
    const response = { status: 200, headers: {}, body: { observed: "again" } };
    const observe = vi.fn(async () => response);
    const result = await observeAnonymousPublicRepo(inaccessible, route, observe);
    expect(result.response).toBe(response);
    expect(result.observedAt).toBeGreaterThanOrEqual(Date.parse(stored!.checked_at + "Z"));
    expect(observe).toHaveBeenCalledOnce();
    expect(await proof()).toEqual(stored);
    expect(await owners()).toBe(0);
  });

  it.each(["half", "older", "negative", "expired", "legacy"])(
    "keeps normal pre-observation acquisition for a %s edge proof",
    async (kind) => {
      const now = Math.floor(Date.now() / 1000) * 1000;
      const coordinator = publicProofCoordinatorStub(env);
      const prior = (await coordinator.tryAcquirePublication(resource))!;
      const checkedAt = sqliteTimestamp(
        now - (kind === "half" ? 150_000 : kind === "older" ? 151_000 : 0),
      );
      expect(
      ...
      expect(await coordinator.completePublication(prior, "shared")).toBe(true);
      if (kind === "expired" || kind === "legacy") {
        const edge = await readEdgeJSON<Record<string, unknown>>(
          PUBLIC_PROOF_EDGE_NAMESPACE,
          "openclaw/octopool",
        );
        await writeEdgeJSON(
          ...
          "openclaw/octopool",
          {
            ...edge,
            ...(kind === "expired"
              ? { expires_at: sqliteTimestamp(now - 1_000) }
              : { protocol_epoch: "old" }),
          },
          60,
        );
      ...
      const stored = await proof();
      expect(stored!.publication_id).toBeGreaterThan(prior.id);
      expect(stored!.is_public).toBe(1);
      expect(stored!.checked_at).toBe(sqliteTimestamp(now));
      expect(await owners()).toBe(0);
    },
  );

  it("does not let skipped warming make an older proof cover a newer cache body", async () => {
    const coordinator = publicProofCoordinatorStub(env);
    const prior = (await coordinator.tryAcquirePublication(resource))!;
    expect(
      await storePublicRepoProof(
        ...
        "octopool",
        true,
        sqliteTimestamp(Date.now() - 10_000),
        prior,
      ),
    ).toBe("shared");
    await coordinator.completePublication(prior, "shared");
    await observeAnonymousPublicRepo(env, route, async () => positive());
    expect((await proof())!.publication_id).toBe(prior.id);
    const fetcher = vi.fn(async () => Response.json({ private: false }));
    vi.stubGlobal("fetch", fetcher);
    await ensurePublicGitHubRepo(egress(), route, sqliteTimestamp(Date.now()));
    expect(fetcher).toHaveBeenCalledOnce();
    expect((await proof())!.publication_id).toBeGreaterThan(prior.id);
  });

  it("leaves a newer authoritative denial intact when optional warming sees an older positive edge", async () => {
    await observeAnonymousPublicRepo(env, route, async () => positive());
    const positiveEdge = await readEdgeJSON(PUBLIC_PROOF_EDGE_NAMESPACE, "openclaw/octopool");
    const coordinator = publicProofCoordinatorStub(env);
    const denial = (await coordinator.tryAcquirePublication(resource))!;
    expect(
      await storePublicRepoProof(
        env,
        "openclaw",
    ...
    await coordinator.completePublication(denial, "shared");
    await writeEdgeJSON(PUBLIC_PROOF_EDGE_NAMESPACE, "openclaw/octopool", positiveEdge, 300);
    await observeAnonymousPublicRepo(env, route, async () => positive());
    expect(await proof()).toMatchObject({ publication_id: denial.id, is_public: 0 });
    await clearEdge();
